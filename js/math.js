...
const circleGrid = (r1, r2, w1, w2) => {  // Parameterbereich: Kreisring um 0
    const GRIDSIZE_R = 30
    const GRIDSIZE_W = 70
...
const evalWeierstrass = (grid, f, g) => {
    ...
    const data = range(grid.length).map(() => range(grid[0].length).map(x=> [0, 0, 0]))
